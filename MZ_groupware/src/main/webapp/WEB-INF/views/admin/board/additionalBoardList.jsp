			...
			<div class="card card-custom gutter-b example example-compact" style="height:100%">
				<div class="card-header flex-wrap border-0 pt-6 pb-0">
					<div class="card-title">
						...
						<button class="btn btn-primary font-weight-bolder" onclick="javascript:OpenWindow('<%=request.getContextPath()%>/board/additional/registForm.do?boardCode=${menu.menuCode }','게시글 등록',800,700);">
							...
							<div class="my-2 my-md-0">
								<div class="input-group row">
									<select class="form-control col-md-3" name="perPageNum" id="perPageNum"
									        onchange="list_go(1, '<%=request.getContextPath()%>/board/additional/list.do?tempStr=${menu.menuCode }');">
									...
									<span class="input-group-append">
										<button class="btn btn-light-primary px-6 font-weight-bold" type="button" id="searchBtn" data-card-widget="search" onclick="list_go(1, '<%=request.getContextPath()%>/board/additional/list.do?tempStr=${menu.menuCode }');">
							...
							<tbody>
								...
								<tr>
									...
									<td class="text-dark text-hover-primary mb-1" style="text-align: left; color: #007bff; cursor:pointer;"
										onclick="javascript:OpenWindow('<%=request.getContextPath() %>/board/additional/detail.do?addNo=${board.addNo }','상세보기',800,700);">
											<span >${board.addTitle }</span> &nbsp;
									    <c:if test="${!empty board.attachList }">
											<i class="nav-icon fas fa-file"></i>
										</c:if>
									</td>
									<td class="text-center align-middle">${board.empName }</td>
									<td class="text-center align-middle">
										<fmt:parseDate value="${board.addRegdate }" var="regDate" pattern="yyyy-MM-dd" />
										<fmt:formatDate value="${regDate }" pattern="yyyy-MM-dd" />
									</td>
									<td class="text-center align-middle">${board.addViewcnt }</td>
								</tr>
								</c:forEach>
							</tbody>
						</table>
				...
				<div class="card-footer p-0">
					...
					<%@ include file="/WEB-INF/views/admin/board/pagination.jsp" %>				
				</div>
			</div><!--end::Card-->
		<!--end::Container-->
		</div>
