		...
		<div class="container">
			<div class="card card-custom gutter-b example example-compact">
				<div class="card-header">
					<h3 class="card-title">${menu.menuName } 게시글 수정</h3>
				...
				<!--begin::Form-->
				<div class="card-body">
					<form enctype="multipart/form-data" role="form" method="post" action="<%=request.getContextPath()%>/board/additional/modify.do" name="modifyForm">
						...
						<div class="form-group row">
							...
							<div class="col-10">
								<input class="form-control" type="text" id="addTitle" name="addTitle" value="${board.addTitle}" />
							</div>
						</div>
						<div class="form-group row">
							<div class="col-12">
								<textarea id="addContent" name="addContent" value="test" style="display: none;"></textarea>
								<div class="summernote" id="kt_summernote_1">${board.addContent}</div>
							</div>
						</div>
 	...
 	$('body').removeClass(
  	...
  	$('a[name="attachedFile"] > button').click(function(event){ // 동적으로 생긴 태그가 아니므로 바로 이벤트 적용 가능.
		...
		$('#addFileBtn').on('click',function(event){
			...
			if(attachCount >= ${addType.attachNum}){
				alert("파일 추가는 ${addType.attachNum}개까지만 가능합니다.");
				return;
			}	
