		...
		<div class="container-fluid">
			<div class="card card-custom gutter-b example example-compact">
				<div class="card-header flex-wrap border-0 pt-6 pb-0">
					<h3 class="card-title">${menu.menuName } 게시글 작성</h3>
				...
				<div class="card-body">
					<form enctype="multipart/form-data" role="form" method="post" action="<%=request.getContextPath()%>/board/additional/regist.do" name="registForm">
						<input type="hidden" id="boardCode" name="boardCode" value="${menu.menuCode }" />
						<input type="hidden" id="empNo" name="empNo" value="${loginUser.empNo}" />
						<div class="form-group row">
							<label for="addTitle" class=" col-2 col-form-label">제목</label>
							<div class="col-10">
								<input class="form-control" type="text" id="addTitle" name="addTitle">
							</div>
						</div>
						<div class="form-group row">
							<div class="col-12">
								<textarea id="addContent" name="addContent" style="display: none;"></textarea>
								<div class="summernote" id="kt_summernote_1"></div>
							</div>
						</div>
						
						<div class="card card-custom card-stretch uppy" style="border: 1px solid #E4E6EF;">
                            ...
                            <div class="card-body uppy-list fileInput">
                            </div>
                        </div>
					</form>
				</div><!-- card-body -->
...
<script>
	$('body').removeClass(
   	...
   	function addFile_go(){
   		if($('input[name="uploadFile"]').length >= ${addType.attachNum}){
   			alert("파일추가는 ${addType.attachNum}개까지만 가능합니다.");
   			return;
   		}
   	...
   	function regist_go(){
   		...
   		if($("input[name='addTitle']").val()==""){
   			alert("제목은 필수입니다.");
   			$("input[name='addTitle']").focus();
   			return;
   		}
	   	// textarea에 입력 내용 삽입
		$("#addContent").val($(".summernote").summernote("code"));
