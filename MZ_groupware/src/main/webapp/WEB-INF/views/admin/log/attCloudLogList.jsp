            ...
            <div class="card card-custom" style="height:100%">
                ...
                <div class="card-body p-3">
                    ...
                    <div class="d-flex justify-content-between mt-3 mb-6">
	                    <button class="btn btn-primary font-weight-bolder btn-sm" data-toggle="tooltip" title="완전삭제파일의 기록과 실제 파일을 삭제합니다." onclick="remove_file();">
	                        첨부파일 정리
	                    </button>
						...
						<table class="table">
							<thead>
								<tr>
									...
									<th class="text-center" style="width: 200px;">파일위치</th>
									<th class="text-center">파일명</th>
									<th class="text-center" style="width: 100px;">삭제여부</th>
									<th class="text-center" style="width: 100px;">첨부자</th>
									<th class="text-center" style="width: 150px;">첨부일</th>
								</tr>
							</thead>
							<tbody>
								<c:forEach items="${dataMap.cloudList}" var="cloud">
									<tr>
										<td class="text-center align-middle">${cloud.cloudNo}</td>
										<td class="text-center align-middle">
											<c:choose>
												<c:when test="${cloud.isDel eq 'X'}">완전삭제파일</c:when>
												<c:otherwise>${cloud.cloudFolderName}</c:otherwise>
											</c:choose>
											
										</td>
										<td class="align-middle">${cloud.cloudFilename}</td>
										<td class="text-center align-middle">
											<c:choose>
												<c:when test="${cloud.isDel eq 'N'}"></c:when>
												<c:otherwise>삭제</c:otherwise>
											</c:choose>
										</td>
										<td class="text-center align-middle" data-toggle="tooltip" data-placement="bottom" title="${cloud.deptName}&nbsp;${cloud.empName}&nbsp;${cloud.psName}(${cloud.cloudAttacher})">
											${cloud.empName}
										</td>
										<td class="text-center align-middle">
											<fmt:formatDate value="${cloud.cloudDate}" pattern="yyyy-MM-dd HH:mm"/>
										</td>
									</tr>
								</c:forEach>
							</tbody>
...
<script type="text/javascript">
	function remove_file() {
		location.href = "<%=request.getContextPath()%>/admin/log/attach/cloud/deleteFile.do";
	}
</script>
</body>
