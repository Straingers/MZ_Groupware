            ...
            <div class="card card-custom" style="height:100%">
            	<div class="card-header flex-wrap border-0 pt-6 pb-0">
					<div class="card-title">
						...
						<button class="btn btn-primary font-weight-bolder" onclick="javascript:OpenWindow('<%=request.getContextPath()%>/office/pds/registForm.do','게시글 작성',1200,800);">
                ...
                <div class="card-body p-3">
                    <div class="table-responsive">
						<table class="table overflow-auto">
							...
							<tbody>
								<c:if test="${fn:length(dataMap.pdsList) eq 0}">
									<tr>
										<td class="text-center" colspan="5">데이터가 없습니다.</td>
									</tr>
								</c:if>
								<c:forEach items="${dataMap.pdsList}" var="pds"  varStatus="i">
									<tr class="text-dark text-hover-primary mb-1 font-size-lg" 
										onclick="javascript:OpenWindow('<%=request.getContextPath()%>/office/pds/detail.do?pdsNo=${pds.pdsNo}','자료실 상세',800,700);" style="cursor: pointer;">
										...
										<td class="text-center align-middle">${pds.empName}</td>
										<td class="text-center align-middle">${pds.regdate}</td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
