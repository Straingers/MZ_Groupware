            	...
            	<div class="card-header flex-wrap border-0 pt-6 pb-0">
					<div class="card-title">
						...
						<c:if test="${loginUser.authority eq 'ROLE_ADMIN'}">
							<button class="btn btn-primary font-weight-bolder" onclick="javascript:OpenWindow('<%=request.getContextPath()%>/office/rules/registForm.do','게시글 작성',1200,800);">
                ...
                <div class="card-body p-3">
                    <div class="table-responsive">
						<table class="table overflow-auto">
							...
							<tbody>
								<c:if test="${fn:length(dataMap.officeList) eq 0}">
									<tr>
										<td class="text-center" colspan="5">데이터가 없습니다.</td>
									</tr>
								</c:if>
								<c:forEach items="${dataMap.officeList}" var="office" varStatus="i">
									<tr onclick="javascript:OpenWindow('<%=request.getContextPath()%>/office/rules/detail.do?officeNo=${office.officeNo}','회사내규 상세',800,700);" style="cursor: pointer;">
										...
										<td class="text-center align-middle">${office.adminName}</td>
										<td class="text-center align-middle">${office.officeRegdate}</td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
...
<form id="form">
	<input id="officeNo" name="officeNo" type="hidden"/>
