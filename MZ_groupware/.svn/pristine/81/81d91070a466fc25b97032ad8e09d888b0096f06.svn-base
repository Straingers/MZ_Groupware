...
<c:set var="resultList" value="${dataMap.approvalDocList}" />
            ...
            <div class="card card-custom" style="height:100%">
				<div class="card-body p-3">
				 	<div class="d-flex justify-content-between mt-3 mb-6">
						<button class="btn btn-primary font-weight-bolder" onclick="OpenWindow('<%=request.getContextPath()%>/admin/approval/intestat/normal/registForm.do','등록',1200,800);">
						...
						<table class="table">
							<thead>
								<tr>
									...
									<th class="text-center" style="width: 100px;">양식코드</th>
...
<%-- 											<c:out value="${resultBean.taskDocId}" /> --%>
										...
										<td class="text-center align-middle"><c:out value="${resultBean.docId}" /></td>
										<td class="text-center align-middle"><c:out value="${resultBean.docName}" /></td>
										<td class="text-center align-middle"><c:out value="${resultBean.isUse eq 'Y' ? '사용' : '미사용'}" /></td>
										<td class="text-center align-middle">
											<c:if test="${resultBean.isUse eq 'Y'}">
												<a href="javascript:fn_update('${resultBean.docId}', 'N');" class="btn btn-light-dark font-weight-bold btn-sm">사용해제</a>
											</c:if>
											<c:if test="${resultBean.isUse eq 'N'}">
												<a href="javascript:fn_update('${resultBean.docId}', 'Y');" class="btn btn-light-primary font-weight-bold btn-sm">사용하기</a>
											</c:if>
											<a href="javascript:OpenWindow('<%=request.getContextPath()%>/admin/approval/intestat/normal/modifyForm.do?docId=${resultBean.docId}','수정하기',1200,900);" class="btn btn-icon btn-primary btn-sm mr-2">
												<i class="flaticon2-edit"></i>
											</a>
											<a href="javascript:fn_remove('${resultBean.docId}');" class="btn btn-icon btn-danger btn-sm">
...
<script>
function fn_update(docId, use) {
	$('#docId').val(docId);
	$('#isUse').val(use);
	$('#form2').attr("action", '<%=request.getContextPath()%>/admin/approval/intestat/normal/update');
...
function fn_remove(docId) {
	if (confirm('해당 결재 양식을 삭제하시겠습니까?')) {
		$('#docId').val(docId);
		$('#form2').attr("action", '<%=request.getContextPath()%>/admin/approval/intestat/normal/remove');
		$('#form2').attr("method", "POST");
		$('#form2').submit();
	}
}
