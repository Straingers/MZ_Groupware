            ...
            <div class="card card-custom" style="height:100%">
                <div class="card-header card-header-tabs-line p-2">
                    <div class="card-toolbar">
                        <ul class="nav nav-tabs nav-bold nav-tabs-line">
                            <li class="nav-item" onclick="parent.goPage('task/share/uncheck/list.do','M030100');">
                                <a class="nav-link <c:if test="${pageType eq 'uncheck'}">active</c:if>" data-toggle="tab" href="#">
                                    <span class="nav-icon"><i class="fas fa-tasks"></i></span>
                                    <span class="nav-text">미확인내역</span>
                                </a>
                            </li>
                            <li class="nav-item" onclick="parent.goPage('task/share/receive/list.do','M030100');">
                            ...
                            <li class="nav-item" onclick="parent.goPage('task/share/send/list.do','M030100');">
                            ...
                            <li class="nav-item" onclick="parent.goPage('task/share/temp/list.do','M030100');">
                ...
                <div class="card-body p-3">
                    <div class="d-flex justify-content-between mt-3 mb-6">
	                    <button class="btn btn-primary font-weight-bolder btn-sm" onclick="OpenWindow('<%=request.getContextPath()%>/task/share/registForm.do','작성',1000,700);">
	                    ...
	                    <div class="form-inline">
							<div class="input-group row">
								...
								<select class="form-control col-md-3" name="searchType" id="searchType">
									...
									<option value="w" ${pageMaker.cri.searchType eq 'w' ? 'selected':'' }>발신인</option>
						...
						<table class="table">
							<thead>
								<tr>
									...
									<th class="text-center" style="width: 150px;">확인/수신/참조</th>
								...
								<c:forEach items="${dataMap.taskList}" var="resultBean" varStatus="i">
									<tr>
										...
										<td class="align-middle">
											<a class="text-dark text-hover-primary mb-1 font-size-lg" href="javascript:OpenWindow('<%=request.getContextPath()%>/task/share/detail.do?taskNo=${resultBean.taskNo}','업무공유 상세',1000,700);">
	                                 			${resultBean.taskTitle }
	                             			</a>
                             			</td>
										<td class="text-center align-middle">
											<c:if test="${resultBean.receiverSign eq null}">0</c:if>
											<c:if test="${resultBean.receiverSign ne null}">${resultBean.receiverSign}</c:if>
											 / ${resultBean.receiverCount}
											 / <c:if test="${resultBean.ccCount eq null}">0</c:if><c:if test="${resultBean.ccCount ne null}">${resultBean.ccCount}</c:if>
										 </td>
