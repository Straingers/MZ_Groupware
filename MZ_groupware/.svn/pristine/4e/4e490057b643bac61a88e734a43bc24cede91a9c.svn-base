        ...
        <div class="container-fluid">
            ...
            <div class="card card-custom gutter-b example example-compact">
                <div class="card-header">
                    <h3 class="card-title">일일보고 임시저장 수정</h3>
                ...
                <div class="card-body">
                    <form enctype="multipart/form-data" method="post" name="registForm">
                        ...
                        <input type="hidden" name="taskSubtype" value="${task.taskSubtype}"/>
                    </form>
                </div>
            </div><!-- card-body -->
        </div><!-- card -->
...
$(document).ready(function(){
	...
	var subType = $('[name=taskSubtype]').val();
	$('input[name=taskDocType]:radio[value="' + subType + '"]').prop('checked', true);
	
	var type = {taskDocType : subType}
	$.ajax({
		...
		success: function(data) {
			...
			if(data.cc.length > 0) {
				for(var i = 0; i < data.cc.length; i++) {
					cc += '<span class="badge badge-pill badge-primary mr-2">' + data.cc[i].empName + '</span>'
...
function temp_go() {
	 ...
	 $("#taskContent").val($(".summernote").summernote("code"));
	 $("form[name='registForm']").attr("action", "<%=request.getContextPath()%>/task/daily/modify");
	 document.registForm.submit();
 }
	
 function remove_go() {
	  if(confirm("삭제하시겠습니까?")) {
		  $("form[name='registForm']").attr("action", "<%=request.getContextPath()%>/task/daily/remove");
	  }
