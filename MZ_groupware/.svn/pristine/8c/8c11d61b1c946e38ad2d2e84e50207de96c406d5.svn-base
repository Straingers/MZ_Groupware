            ...
            <div class="card card-custom" style="height:100%">
                <div class="card-header card-header-tabs-line p-2">
                    <div class="card-toolbar">
                        <ul class="nav nav-tabs nav-bold nav-tabs-line">
                            <li class="nav-item" onclick="parent.goPage('task/important/send/list.do','M030400');">
                            ...
                            <li class="nav-item" onclick="parent.goPage('task/important/receive/list.do','M030400');">
                ...
                <div class="card-body p-3">
                    <div class="d-flex justify-content-between mt-3 mb-6">
	                    <button class="btn btn-primary font-weight-bolder btn-sm" onclick="OpenWindow('<%=request.getContextPath()%>/task/share/registForm.do','작성',1200,800);">
						...
						<table class="table">
							<thead>
								<tr>
									...
									<th class="text-center" style="width: 100px;"><i class="fas fa-star text-warning"></i></th>
							...
							<tbody>
								...
								<c:forEach items="${dataMap.taskList}" var="resultBean" varStatus="i">
									<tr>
										...
										<td class="text-center align-middle" id="important">
											<c:if test="${resultBean.taskImportant eq 'Y'}">
												<a name="starIcon${resultBean.taskNo}" href="javascript:fn_update('${resultBean.taskNo}', 'N');">
													<i class="fas fa-star text-warning"></i>
												</a>
											</c:if>
											<c:if test="${resultBean.taskImportant eq 'N'}">
										...
										<td>
											...
											<c:choose>
												<c:when test="${sub eq '업무공유'}">
													<span class="label label-lg font-weight-bolder label-rounded label-info" style=" width: 70px;">업무공유</span>
												</c:when>
												...
												<c:when test="${sub eq '주간보고'}">
													<span class="label label-lg font-weight-bolder label-rounded label-primary" style=" width: 70px;">주간보고</span>
												</c:when>
												<c:when test="${sub eq '월간보고'}">
													<span class="label label-lg font-weight-bolder label-rounded label-warning" style=" width: 70px;">월간보고</span>
												</c:when>
											</c:choose>
										</td>
										<td class="align-middle">
											<c:if test="${resultBean.taskSubtype eq '업무공유'}">
											<a class="text-dark text-hover-primary mb-1 font-size-lg" href="javascript:OpenWindow('<%=request.getContextPath()%>/task/share/detail.do?taskNo=${resultBean.taskNo}','업무공유 상세',1500,1000);">
	                                 			${resultBean.taskTitle }
	                             			</a>
											</c:if>
											<c:if test="${resultBean.taskSubtype ne '업무공유'}">
											<a class="text-dark text-hover-primary mb-1 font-size-lg" href="javascript:OpenWindow('<%=request.getContextPath()%>/task/daily/detail.do?taskNo=${resultBean.taskNo}','일일보고 상세',1500,1000);">
	                                 			${resultBean.taskTitle }
	                             			</a>
											</c:if>
										</td>
...
function fn_update(taskNo, important) {
	...
	$.ajax({
	    url:'<%=request.getContextPath()%>/task/important/sendUpdate', 
	    type:'post', // 메소드(get, post, put 등)
	    data: JSON.stringify(form),
	    contentType: 'application/json; charset=utf-8',
	    success: function(data) {
	    	if(data.important == 'Y') {
		        $("#important > [name='starIcon"+taskNo+"']").remove();
