        ...
        <div class="container">
            <div class="card card-custom gutter-b example example-compact">
                <div class="card-header">
                    <h3 class="card-title">신입사원 교육자료 수정</h3>
                ...
                <div class="card-body">
                    <form enctype="multipart/form-data" method="post" name="modifyForm">
                    	<input type="hidden" name="otNo" id="otNo" value="${ot.otNo }">
                        <div class="form-group row">
                            <label for="otTitle" class="col-2 col-form-label text-center">제목</label>
                            <div class="col-10">
                                <input id="otTitle" name="otTitle" type="text" class="form-control" value="${ot.otTitle }"/>
                            </div>
                        </div>
                        <div class="form-group row">
                            <label class="col-2 col-form-label text-center">작성자</label>
                            <div class="col-10">
		                        <div class="form-control" style="display: flex;align-items: center;font-weight: bold;" readonly>${ot.adminName}</div>
                            </div>
                        </div>
                        <div class="form-group row">
                            <div class="col-12">
                               	<div class="summernote" id="kt_summernote_1">${ot.otContent }</div>
                                <textarea id="otContent" name="otContent" style="display: none;"></textarea>
                            </div>
                        </div>
                        <div class="card card-custom card-stretch" style="border: 1px solid #E4E6EF;">
                            ...
                            <div class="card-body fileInput">
                            	<c:forEach items="${ot.attachList}" var="attach">
									<div class="inputRow attach-item">
										<a class="text-dark text-hover-primary mb-1 font-size-lg" 
											...
											href="<%=request.getContextPath()%>/office/ot/getFiles.do?attachNo=${attach.attachNo}" >
...
<script>
	function modify_go() {
		...
		$("#otNo").val(${ot.otNo});
		if($("input[name='otTitle']").val()==""){
    		alert(input.name+"은 필수입니다.");
 	      $("input[name='otTitle']").focus();
 	      return;
    	}
		
		// textarea에 입력 내용 삽입
		$("#otContent").val($(".summernote").summernote("code"));
  	
		if($("[name='otContent']").val() == "<p><br></p>"){
   			alert("내용은 필수입니다.");
   			$("[name='otContent']").focus();
   			return;
   		}
		formObj.attr({
			"action":"modify.do"
	...
	$('#addFileBtn').on('click',function(event){
		...
		if(attachCount >= 1
	    		&& $('input[name="uploadFile"]').val().split('.')[1] != "pdf"){
			alert("파일 추가는 pdf확장자를 가진 파일로 1개만 가능합니다.");
			return;
		}	
